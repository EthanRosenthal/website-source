...
While this monitoring process was certainly doable, it was certainly _annoying_, as well. First and foremost, I wanted to build a service that made this whole process extremely simple. Simpler than writing my own ETL pipelines. Secondly, I wanted to be able to interactively slice and dice my model's performance across a range of dimensions over time. How do new customer's predictions compare to recurring customer's? How have these two segments' predictions changed over time? 
...
<!-- ```json
// Example binary classification prediction event
{
  "context": {
    "logged_in": true,
    "mobile": false,
  ...
  "model_version": "1.3.2",
  "score": 0.7,
  "service_name": "cart-abandoner",
  "threshold": 0.5,
  "timestamp": 1568848171.193525
}
```

On my side, I built out a Flask API using [connexion](https://github.com/zalando/connexion) which validated and accepted events. I then utilized a bunch of AWS managed services for collecting, organizing, and querying data on S3. Specifically, I dropped events into a Kinesis queue, where they then landed on S3, got fanned out by Lambda functions, and converted to parquet. I ran AWS Glue crawlers to create tables to be queried by Athena. Nightly batch jobs joined the prediction and outcome events. Lastly, I built a Flask CRUD app as a UI. It handled account creation, service tracking management, and interactive dashboards of model performance (via [Panel](https://panel.holoviz.org/) because I still don't know javascript). -->
